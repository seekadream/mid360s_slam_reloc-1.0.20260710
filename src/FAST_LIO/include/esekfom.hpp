...
#pragma omp parallel for
		...
		void update_with_reg(double R,Eigen::Matrix4d & reg_pose){
			...
			Eigen::Matrix3d R_reg = reg_pose.block<3,3>(0,0);
			Eigen::JacobiSVD<Eigen::Matrix3d> svd(R_reg, Eigen::ComputeFullU | Eigen::ComputeFullV);
			R_reg = svd.matrixU() * svd.matrixV().transpose();
			if(R_reg.determinant() < 0) R_reg = -R_reg;
			Sophus::SO3<double> reg_so3 = Sophus::SO3<double>(R_reg);
			//H矩阵6*18
			...
			x_.rot = x_.rot * Sophus::SO3<double>::exp(updated_err.block<3, 1>(3, 0));//更新旋转
