...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)
{
  ...
  init_state.grav = - mean_acc / acc_norm * G_m_s2;
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    double acc_norm_safe = mean_acc.norm();
    if (acc_norm_safe < 1e-6) acc_norm_safe = 1e-6;
    acc_avr  = acc_avr * G_m_s2 / acc_norm_safe;
    ...
    acc_s_last  = V3D(tail->linear_acceleration.x, tail->linear_acceleration.y, tail->linear_acceleration.z) * G_m_s2 / acc_norm_safe;   
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
    {
      ...
      M3D R_i(R_imu * Sophus::SO3<double>::exp(angvel_avr * dt).matrix() );
...
void ImuProcess::Process(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI::Ptr &cur_pcl_un_)
{
  ...
  if (imu_need_init_)   
  {
    ...
    {
      double acc_norm_safe2 = mean_acc.norm();
      if (acc_norm_safe2 < 1e-6) acc_norm_safe2 = 1e-6;
      cov_acc *= pow(G_m_s2 / acc_norm_safe2, 2);
